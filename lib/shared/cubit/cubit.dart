...
class AppCubit extends Cubit<AppStates>{
    ...
    openDatabase(
      ...
      onCreate: (db, version){
        ...
        db.execute(
          ...
          print('Error when creating database ${err.toString()}!');
        });
      },
  ...
  void insertToDatabase ({
    ...
    await database.transaction(
          ...
          txn.rawInsert(
            ...
            print('$value inserted successfully in table');
            emit(AppInsertToDatabaseState());
          }).catchError((err){
            print('Error while inserting record in table tasks: ${err.toString()}');
          });
          return null;
        }).then((value) {
  ...
  @required int id,
    ...
    await database.rawDelete(
        ...
        [id]).then((value) {
          print('task $value was deleted successful');
  ...
  void getDataFromDatabase (database) async {
    ...
    database.rawQuery('SELECT * FROM tasks').then((value){
      ...
      value.forEach((element) {
        if (element['status'] == 'new task') newTasks.add(element);
        else if (element['status'] == 'done task') doneTasks.add(element);
        else if (element['status'] == 'archived task') archiveTasks.add(element);
      });
      emit(AppGetDatabaseState());
      print('get database table');
    });
  }
